...
# Configuring curl
RUN cd ${SRCDIR}/curl-* \
    && LDFLAGS="-static -L${TARGETDIR}/lib/" CPPFLAGS="-I${TARGETDIR}/include/" LIBS="-lbrotlidec-static -lbrotlicommon-static" PKG_CONFIG="pkg-config --static" ./configure --disable-shared --enable-ares --with-ca-fallback --with-libssh2 --with-gssapi
# --enable-ares --enable-alt-svc --enable-mqtt --with-ssl --with-gnutls --with-mbedtls --with-nss --with-libmetalink
# Making curl
RUN cd ${SRCDIR}/curl-* \
